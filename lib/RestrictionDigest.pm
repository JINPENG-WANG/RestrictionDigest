...
sub double_digest {
  ...
  # Front Enzyme cut seq stores.
  my $front_enzyme_cut_seq_count=0;
  for my $front_base (@front_enzyme_rec_seq){
    $front_enzyme_rec_start++;
    if($front_base=~/\|/){
      $front_enzyme_cut_loc=$front_enzyme_rec_start;
    }
    else{
		$front_enzyme_cut_seq_count++;
		if(exists $degenerates{$front_base}){
			for my $sub_base (@{$degenerates{$front_base}}){
				push @{$front_enzyme_cut_seq{$front_enzyme_cut_seq_count}}, $sub_base;
			}
		}else{
			push @{$front_enzyme_cut_seq{$front_enzyme_cut_seq_count}}, $front_base;
		}
	}
  }

  # Later Enzyme cut seq stores.
  my $later_enzyme_cut_seq_count=0;
  for my $later_base (@later_enzyme_rec_seq){
	$later_enzyme_rec_start++;
	if($later_base=~/\|/){
	  $later_enzyme_cut_loc=$later_enzyme_rec_start;
	}
	else{
		$later_enzyme_cut_seq_count++;
		if(exists $degenerates{$later_base}){
			for my $sub_base (@{$degenerates{$later_base}}){
				push @{$later_enzyme_cut_seq{$later_enzyme_cut_seq_count}}, $sub_base;
			}
		}else{
			push @{$later_enzyme_cut_seq{$later_enzyme_cut_seq_count}}, $later_base;
		}
	}
  }
  ...
  while(<$ref_fh>){
    ...
    if($_=~/^(>\S+)/){
      ...
      if($seq_name_length !=0){
		...
		my @front_enzyme_arr= map {
			my $k = $_;
			[map "$_", @{$front_enzyme_cut_seq{$k}}];
		} sort {$a<=>$b} keys %front_enzyme_cut_seq;

		my @front_out = getCartesian(@front_enzyme_arr);
		for my $front_one (@front_out) {
			my @front_rec_bases = @$front_one;
			my $front_rec_seq = join("", @front_rec_bases);
			my $loc_in_array;
			$loc_in_array=index($seq_for_search, $front_rec_seq);
			unless($loc_in_array == -1){
				push @front_enzyme_locs, $loc_in_array;
			}
			while($loc_in_array !=-1){
				$loc_in_array=index($seq_for_search, $front_rec_seq, $loc_in_array+1);
				unless($loc_in_array == -1){
					push @front_enzyme_locs, $loc_in_array;
				}
			}
		...
		my @later_enzyme_arr = map {
			my $k = $_;
			[map "$_", @{$later_enzyme_cut_seq{$k}}];
		} sort {$a<=>$b} keys %later_enzyme_cut_seq;
		
		my @later_out = getCartesian(@later_enzyme_arr);
		for my $later_one(@later_out) {
			my @later_rec_bases = @$later_one;
			my $later_rec_seq = join("", @later_rec_bases);
			my $loc_in_array;
			$loc_in_array = index ($seq_for_search, $later_rec_seq);
			unless ($loc_in_array == -1) {
				push @later_enzyme_locs, $loc_in_array;
			}
			while($loc_in_array != -1){
				$loc_in_array = index($seq_for_search, $later_rec_seq, $loc_in_array+1);
				unless($loc_in_array == -1){
					push @later_enzyme_locs, $loc_in_array;
				}
			}
...
sub single_digest {
  ...
  my $enzyme_cut_seq_count=0;

  for my $base (@enzyme_rec_seq){
    $enzyme_rec_start++;
    if($base=~/\|/){
      $enzyme_cut_loc=$enzyme_rec_start;
    }
    else{
		$enzyme_cut_seq_count++;
		if(exists $degenerates{$base}){
			for my $sub_base(@{$degenerates{$base}}) {
				push @{$enzyme_cut_seq{$enzyme_cut_seq_count}}, $sub_base;
			}
		}
		else{
			push @{$enzyme_cut_seq{$enzyme_cut_seq_count}}, $base;
		}
	}
  }
  ...
  while(<$ref_fh>){
    ...
    if($_=~/^>/){
      ...
      if($seq_name_length !=0){
		...
		my @enzyme_arr = map {
			my $k = $_;
			[map "$_", @{$enzyme_cut_seq{$k}}];
		
		}  sort {$a<=>$b} keys %enzyme_cut_seq;
		
		my @arr_out = getCartesian(@enzyme_arr);
		for my $arr_one (@arr_out) {
			my @single_rec_bases= @$arr_one;
			my $single_rec_seq = join("", @single_rec_bases);
			my $loc_in_array;
			$loc_in_array = index($seq_for_search, $single_rec_seq);
			unless($loc_in_array == -1){
				push @enzyme_locs, $loc_in_array;
			}
			while($loc_in_array != -1){
				$loc_in_array = index($seq_for_search, $single_rec_seq, $loc_in_array+1);
				unless ($loc_in_array == -1) {
					push @enzyme_locs, $loc_in_array;
				}
			}
